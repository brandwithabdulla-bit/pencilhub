...
export default function Footer({ settings }: FooterProps) {
  // Build active dynamic social links from settings
  const socialLinks = [
    { name: "Youtube", href: settings?.socialLinks?.youtube || "https://www.youtube.com/@pencilhub" },
    { name: "Instagram", href: settings?.socialLinks?.instagram || "https://www.instagram.com/pencil_hub/" },
    { name: "X (Twitter)", href: settings?.socialLinks?.twitter || "https://x.com/miqu_mk" },
    { name: "Linkedin", href: settings?.socialLinks?.linkedin || "https://linkedin.com/company/pencilhub" },
    { name: "Behance", href: settings?.socialLinks?.behance || "https://behance.net/pencilhub" },
    { name: "Dribbble", href: settings?.socialLinks?.dribbble || "https://dribbble.com/pencilhub" },
  ];
    ...
    <footer className="relative bg-[#0A0A0A] text-white pt-24 pb-8 overflow-hidden select-none">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-12 gap-12 relative z-10">
        ...
        <div className="md:col-span-6 flex flex-col gap-4">
          ...
          <a
            href={`mailto:${settings?.contactEmail || "pencilhubsocial@gmail.com"}`}
          ...
          >
            {settings?.contactEmail || "pencilhubsocial@gmail.com"}
      ...
      <div className="w-full mt-24 border-t border-white/5 py-4 flex justify-center items-center overflow-hidden select-none pointer-events-none">
        <h2 className="font-display text-[13.5vw] font-black tracking-tighter leading-none text-white/[0.025] uppercase m-0 p-0 font-extrabold">
          {(settings?.siteName || "PENCILHUB").toUpperCase()}
        </h2>
      </div>

      {/* Bottom Copyright info block */}
      <div className="max-w-7xl mx-auto px-6 md:px-12 pt-8 mt-8 border-t border-white/5 flex flex-col sm:flex-row justify-between items-center gap-4 text-[10px] font-mono text-white/30 uppercase tracking-widest">
        <span>© {new Date().getFullYear()} {settings?.siteName || "PencilHub"} Studio. All rights reserved.</span>
